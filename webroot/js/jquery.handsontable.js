...
var weekDatas = new Array();
var shiftCodeDatas = new Array();
var inArrayUser = new Array();
...
(function ($, window, Handsontable) {
  ...
  Handsontable.Core = function (container, settings) {
    ...
    userDatas = settings.userArray;
    weekDatas = settings.weekArray;
    shiftCodeDatas = settings.shiftCodeArray;
    inArrayUser = settings.inArrayUser;
    allUserArray = settings.allUserArray;
    shiftHourArray = settings.shiftHourArray;
    dateArray = settings.dateArray;
    weekMax = parseInt(settings.cols);
    rowMax = parseInt(settings.rows);
	...
	function timeFormat(timeStr) {
		//var trimTime = timeStr.trim();
		var reTime = timeStr.replace(new RegExp(' ', 'gm'), '0');
		var trimTime = reTime.trim();
		var checkTime = trimTime.replace(new RegExp(':', 'gm'), '');
		if(isNaN(checkTime)) {
			return false;
		} else {
			var timeCutStr = trimTime.split(':');
			if(timeCutStr.length > 1) {
				var setHour = timeStrPad(timeCutStr[0], 2);
				var setMin = timeStrPad(timeCutStr[1], 2);
				if(parseInt(setHour, 10) > 23) {
					setHour = '00';
				}
				if(parseInt(setMin, 10) > 59) {
					setMin = '00';
				}
				var newTime = setHour + ':' + setMin;
			} else {
				var strPadTime = timeStrPad(trimTime, 4);
				var setHour = strPadTime[0] + strPadTime[1];
				var setMin = strPadTime[2] + strPadTime[3];
				if(parseInt(setHour, 10) > 23) {
					setHour = '00';
				}
				if(parseInt(setMin, 10) > 59) {
	...
	function timeStrPad(timeStr, max) {// make length of timeStr the same to max;
		if(timeStr.length > max) {
			var newTimeStr = timeStr[0] + timeStr[1] + timeStr[2] + timeStr[3];
			return newTimeStr;
		} else if(timeStr.length == max) {
			return timeStr;
		} else {
			while(timeStr.length < max) {
				timeStr = '0' + timeStr;
			}
			return timeStr;
		}
	...
	function timeArea(timeStr, type) {
		if(type == 'one') {
			var cutStr = timeStr.split('-');
			var startTime = timeTrans(cutStr[0]);
			var endTime = timeTrans(cutStr[1]);
			if(parseInt(endTime, 10) <= parseInt(startTime, 10)) {
				timeStr = false;
			}
		} else {
			var eachTime = timeStr.split(',');
			var errorCount = 0;
			var successCount = 0;
			var newEachTime = '';
			for(var et=0; et<eachTime.length; et++) {
				var cutStr = eachTime[et].split('-');
				var startTime = timeTrans(cutStr[0]);
				var endTime = timeTrans(cutStr[1]);
				if(parseInt(endTime, 10) <= parseInt(startTime, 10)) {
					errorCount++;
	...
	function timeMerge(timeStr) {
		var timeCutStr = timeStr.split(',');
		if(timeCutStr.length > 1) {
			var tmpTimeArray = new Array();
			//var tmpTimeArrayStr = '';
			for(var i=0; i<timeCutStr.length; i++) {
				if(i < 1) {
					tmpTimeArray.push(timeCutStr[i]);
					//tmpTimeArrayStr += timeCutStr[i];
				} else {
					var tmpOrgStartTime = '';
					var tmpOrgEndTime = '';
					var tmpNowStartTime = '';
					var tmpNowEndTime = '';
					var tmpNowTimeCutStr = timeCutStr[i].split('-');
					tmpNowStartTime = timeTrans(tmpNowTimeCutStr[0]);
					tmpNowEndTime = timeTrans(tmpNowTimeCutStr[1]);
					
					//var tmpTimeArrayCutStr = tmpTimeArray.split(',');
					var isColl = false;
					for(var x=0; x<tmpTimeArray.length; x++) {
					//alert(tmpTimeArray[x]);
						var tmpTimeCutStr = tmpTimeArray[x].split('-');
						tmpOrgStartTime = timeTrans(tmpTimeCutStr[0]);
						tmpOrgEndTime = timeTrans(tmpTimeCutStr[1]);
						if((tmpNowStartTime < tmpOrgEndTime)&&(tmpNowEndTime > tmpOrgStartTime)) {
							isColl = true;
							break;
						}
					}
			...
			//var newTimeStr = tmpTimeArray.pop();
			//tmpTimeArray.pop();
			//$('#show').val(tmpTimeArray);
			//var newTimeStr = tmpTimeArrayStr;
			return tmpTimeArray.toString();
		} else {
			return timeStr;
		}
	}
	...
	function timeTrans(timeStr) {
		if((timeStr[0] == '0')&&(timeStr[1] == '0')) {
			return '24' + timeStr[3] + timeStr[4];
		} else {
			return timeStr[0] + timeStr[1] + timeStr[3] + timeStr[4];
		}
	}
	
	function timeDiff(timeStr) {
		var timeCutStr = timeStr.split('-');
		var startTime = timeToMin(timeCutStr[0]);
		var endTime = timeToMin(timeCutStr[1]);
		var timeDiffValue = parseInt(endTime, 10) - parseInt(startTime, 10);
		return timeDiffValue;
	}
	
	function timeToMin(timeStr) {
		var traTime = timeTrans(timeStr);
		var hourStr = traTime[0] + traTime[1];
		var minStr = traTime[2] + traTime[3];
		var minValue = (parseInt(hourStr, 10) * 60) + parseInt(minStr, 10);
		return minValue;
	}
	
	/*function sumHour(startRow, endRow) {
		if(startRow < endRow) {
			var sr = startRow;
			var er = endRow;
		} else {
		...
		for(var r=sr; r<=er; r++) {
			var rowTotalHour = 0;
			for(var c=1; c<(parseInt(weekMax)-1); c++) {
				var tmpShiftCode = datamap.data[r][c].replace('ot_', '');
				if(typeof(shiftHourArray[tmpShiftCode]) != 'undefined') {
					rowTotalHour += parseInt(shiftHourArray[tmpShiftCode], 10);
				}
				//alert(datamap.data[r][c]);
			}
			self.setDataAtCell(r, parseInt(weekMax)-1, rowTotalHour, false, 'edit');
		}
	}*/
    ...
    datamap = {
       ...
       */
		set: function (row, col, value) {
			...
			if($('#isInputShift').val() == '1') {
				var inputSnCutStr = datamap.data[row][0].split('   ');
				var inputSn = inputSnCutStr[1];
				var shiftTypeCutStr = value.split('-');
				if(shiftTypeCutStr.length > 1) {
					var shiftType = 'time';
					$('#isHaveTime').val('1');
				} else {
					var shiftType = 'shift';
				...
				if($('#Schedule_'+inArrayUser[inputSn]+'_'+dateArray[col]+'_userId').length) {
					$('#Schedule_'+inArrayUser[inputSn]+'_'+dateArray[col]+'_userId').val(inArrayUser[inputSn]);
					$('#Schedule_'+inArrayUser[inputSn]+'_'+dateArray[col]+'_date').val(dateArray[col]);
					$('#Schedule_'+inArrayUser[inputSn]+'_'+dateArray[col]+'_type').val(shiftType);
					$('#Schedule_'+inArrayUser[inputSn]+'_'+dateArray[col]+'_codeTime').val(value);
				} else {
					$('#inputDiv').append('<input id="Schedule_'+inArrayUser[inputSn]+'_'+dateArray[col]+'_userId" name="data[Schedule]['+inArrayUser[inputSn]+'_'+dateArray[col]+'][user_id]" value="'+inArrayUser[inputSn]+'" style="display:none;" />');
					$('#inputDiv').append('<input id="Schedule_'+inArrayUser[inputSn]+'_'+dateArray[col]+'_date" name="data[Schedule]['+inArrayUser[inputSn]+'_'+dateArray[col]+'][date]" value="'+dateArray[col]+'" style="display:none;" />');
					$('#inputDiv').append('<input id="Schedule_'+inArrayUser[inputSn]+'_'+dateArray[col]+'_type" name="data[Schedule]['+inArrayUser[inputSn]+'_'+dateArray[col]+'][type]" value="'+shiftType+'" style="display:none;" />');
					$('#inputDiv').append('<input id="Schedule_'+inArrayUser[inputSn]+'_'+dateArray[col]+'_codeTime" name="data[Schedule]['+inArrayUser[inputSn]+'_'+dateArray[col]+'][code_time]" value="'+value+'" style="display:none;" />');
				}
			} else {
				if(col === 0) {
					var inputSnCutStr = value.split('   ');
					var inputSn = inputSnCutStr[1];
					$('#inputDiv').append('<input id="ScheduleUser_'+inArrayUser[inputSn]+'" name="data[ScheduleUser]['+inArrayUser[inputSn]+']" value="'+inArrayUser[inputSn]+'" style="display:none;" />');
					var tmpRank = parseInt(row, 10) + 1;
					$('#inputDiv').append('<input id="ScheduleUserRank_'+inArrayUser[inputSn]+'" name="data[ScheduleUserRank]['+inArrayUser[inputSn]+']" value="'+tmpRank+'" style="display:none;" />');
			...
			//if($('#isInputShift').val() == '1') {
					var rowTotalHour = 0;
					for(var c=1; c<(parseInt(weekMax)-1); c++) {
						var tmpShiftCode = datamap.data[row][c].replace('ot_', '');
						var timeCutStr = datamap.data[row][c].split('-');
						if(typeof(shiftHourArray[tmpShiftCode]) != 'undefined') {
							rowTotalHour += parseFloat(shiftHourArray[tmpShiftCode]);
						} else if(timeCutStr.length > 1) {
							var timeMultyCutStr = datamap.data[row][c].split(',');
							if(timeMultyCutStr.length > 1) {
								//to do multy time
								for(var mt=0; mt<timeMultyCutStr.length; mt++) {
									var minDiff = timeDiff(timeMultyCutStr[mt]);
									var hourDiff = (parseInt(minDiff, 10) / 60).toFixed(1);
									rowTotalHour += parseFloat(hourDiff);
								}
							} else {
								var minDiff = timeDiff(datamap.data[row][c]);
								var hourDiff = (parseInt(minDiff, 10) / 60).toFixed(1);
								rowTotalHour += parseFloat(hourDiff);
							}
						}
					...
					//setDataAtCell(row, parseInt(weekMax)-1, rowTotalHour, false, 'edit');
					var td = grid.getCellAtCoords({row: row, col: parseInt(weekMax)-1});
					td.innerHTML = rowTotalHour.toFixed(1);
					
					datamap.data[row][parseInt(weekMax)-1] = rowTotalHour;
			//}
      },
    ...
    grid = {
      ...
      alter: function (action, coords, toCoords) {
        ...
        switch (action) {
          ...
          case "remove_row":
			 	var tmpUserDataCutStr = datamap.data[coords.row][0].split('   ');
			 	var tmpUserSn = tmpUserDataCutStr[1];
			 	$('#ScheduleUser_'+inArrayUser[tmpUserSn]).remove();
			 	$('#ScheduleUserRank_'+inArrayUser[tmpUserSn]).remove();
			 	delete inArrayUser[tmpUserSn];
      ...
      createRow: function (coords) {
        var tr, c, r, td;
//error: createRow callback don't work.
		...
		empty: function () {
			...
			for (i = 0, ilen = tds.length; i < ilen; i++) {
				...
				if (old !== '' && grid.isCellWritable($td)) {
					if(coords.col == 0) {
						var getSnCutStr = datamap.data[coords.row][coords.col].split('   ');
						var getSn = getSnCutStr[1];
						$('#ScheduleUser_'+inArrayUser[getSn]).remove();
						$('#ScheduleUserRank_'+inArrayUser[getSn]).remove();
						delete inArrayUser[getSn];
					}
					$td.empty();
					self.minWidthFix(tds[i]);
        ...
        function onClick(event) {
          event.stopPropagation();
			...
			function onCut() {
				if (!priv.isCellEdited) {
					setTimeout(function () {
					for(var ui=priv.selStart.row; ui<=priv.selEnd.row; ui++) {
						var tmpSnCutStr = datamap.data[ui][0].split('   ');
						var tmpSn = tmpSnCutStr[1];
						$('#ScheduleUser_'+inArrayUser[tmpSn]).remove();
						$('#ScheduleUserRank_'+inArrayUser[tmpSn]).remove();
						delete inArrayUser[tmpSn];
					}
					selection.empty();
					}, 100);
				}
			...
			function onPaste() {
				if (!priv.isCellEdited) {
					setTimeout(function () {
						var getNameSn = priv.editProxy.val().split('\t').toString().split('   ');
						var realDataCount = 0;
						for(var i=1; i<getNameSn.length; i++) {
							var getSnCutStr = getNameSn[i].split(',');
							var getSn = getSnCutStr[0];
							if(typeof(inArrayUser[getSn]) != 'undefined') {
								realDataCount++;
							}
						}
						...
						if(realDataCount < 1) {
							var errorCount = 0;
							if((priv.selStart.col == '0')||(priv.selEnd.col == '0')) {
								for(var ui=priv.selStart.row; ui<=priv.selEnd.row; ui++) {
									var tmpSnCutStr = datamap.data[ui][0].split('   ');
									var tmpSn = tmpSnCutStr[1];
									if(typeof(inArrayUser[tmpSn]) != 'undefined') {
										errorCount++;
									}
								}
							}
							...
							if(errorCount < 1) {
								...
								//sumHour(priv.selStart.row, priv.selEnd.row);
							} else {
								alert('不允許將貼上的資料覆蓋在其他名單上。');
							}
						} else {
							alert('姓名有重複，故不處理貼上動作。');
						}
					}, 100);
				}
			}
          ...
          if (selection.isSelected()) {
            ...
            switch (event.keyCode) {
              ...
              case 46: /* delete */
                if (!priv.isCellEdited) {
                //////////////////////////////////////////////
                	//to do delete the name and inArrayUser.
	...
	interaction = {
		onMouseDown: function (event) {
			...
			if (event.button === 2 && selection.inInSelection(grid.getCellCoords(this))) { //right mouse button
				...
				//to do select shift code put on it;
				if ($j('#startShift').val() == 1) {
					var coords = grid.getCellCoords(this);
					//$('#show').val(coords.row + '   ' + coords.col);
					//datamap.data[coords.row][coords.col] = $('#selectedShift').val();
					if($j('#setType').val() == '2') {
						selection.empty();
						//sumHour(coords.row, coords.row);
					} else if($j('#setType').val() == '0') {
						self.setDataAtCell(coords.row, coords.col, $('#selectedShift').val(), false, 'edit');
						//sumHour(coords.row, coords.row);
					} else if($j('#setType').val() == '1') {
						var setValue = 'ot_' + $('#selectedShift').val();
						self.setDataAtCell(coords.row, coords.col, setValue, false, 'edit');
						//sumHour(coords.row, coords.row);
					}
					//grid.populateFromArray(coords, $('#selectedShift').val(), null, false, 'edit');
					//datamap.set(coords.row, coords.col, $('#selectedShift').val());
					//grid.updateLegend({row: coords.row, col: coords.col});
					//alert($j('#selectedShift').val());
				}
			}
		},
    ...
    this.init = function () {
      ...
      if (priv.settings.contextMenu) {
        ...
        var allItems = {
          ...
          "sep1": "---------",
          "row_above": {name: "插入至上一筆", disabled: isDisabled},
          "row_below": {name: "插入至下一筆", disabled: isDisabled},
          "sep2": "---------",
          "col_left": {name: "Insert column on the left", disabled: isDisabled},
          "col_right": {name: "Insert column on the right", disabled: isDisabled},
          "sep3": "---------",
          "remove_row": {name: "刪除名單", disabled: isDisabled},
...
//				var autoCutStr = changes.toString().split(',');
//				var autoData = autoCutStr[3];
//				var timeCutStr = autoData.split('-');
//				if(timeCutStr.length > 1) {
					//$('#show').val(timeCutStr[0] + '   ' + timeCutStr[1]);
					//////////////////////////////////////////////
//				} else {
					var typeahead = priv.editProxy.data('typeahead');
						...
						if (autoComplete.match(changes[c][0], changes[c][1], datamap.getAll)) {
							...
							for (var s = 0, slen = source.length; s < slen; s++) {
								if (changes[c][3] === source[s]) {
									if($('#isInputName').val() == '1') {
										var getSnCutStr = changes[c][3].split('   ');
										var getSn = getSnCutStr[1];
										if(typeof(inArrayUser[getSn]) != 'undefined') {
											alert('此姓名已在名單中');
											changes[c][3] = false;
									...
									continue loop;
									////////////////////////////////////////////////////////////////////////////////////
								} else if((changes[c][3] != source[s])&&(changes[c][1] > 0)) {
									var multyTime = changes[c][3].split(',');
									if(multyTime.length > 1) {
									//multy start time to end time.
										var multyTimeStr = '';
										var errorCount = 0;
										var successTimeCount = 0;
										for(var mt=0; mt<multyTime.length; mt++) {
											var timeCutStr = multyTime[mt].split('-');
											if((timeCutStr.length > 1)&&(timeCutStr[0] != '')&&(timeCutStr[1] != '')) {
												if($('#isInputTime').val() == '1') {
													var startTime = timeFormat(timeCutStr[0]);
													var endTime = timeFormat(timeCutStr[1]);
													if((!startTime)||(!endTime)) {
														...
														var isTime = true;
														//changes[c][3] = startTime + '-' + endTime;
													}
													
													if(successTimeCount == 0) {
														if(isTime) {
															multyTimeStr += startTime + '-' + endTime;
															successTimeCount++;
														}
													} else {
														if(isTime) {
										...
										if($('#isInputTime').val() == '1') {
											if(errorCount >= multyTime.length) {
												changes[c][3] = false;
											} else {
												var newTimeArea = timeArea(multyTimeStr, 'multy');
//												changes[c][3] = newTimeArea;
												//to do merge time area.
												var newTimeMergeStr = timeMerge(newTimeArea);
												changes[c][3] = newTimeMergeStr;
											}
										}
										
										continue loop;
									} else {
									//only one start time to end time.
										var timeCutStr = changes[c][3].split('-');
										if((timeCutStr.length > 1)&&(timeCutStr[0] != '')&&(timeCutStr[1] != '')) {
											if($('#isInputTime').val() == '1') {
												var startTime = timeFormat(timeCutStr[0]);
												var endTime = timeFormat(timeCutStr[1]);
												if((!startTime)||(!endTime)) {
													...
													var oneTimeArea = startTime + '-' + endTime;
													var newTimeArea = timeArea(oneTimeArea, 'one');
													changes[c][3] = newTimeArea;
												}
												//changes[c][3] = '456';
											}
      ...
      for (i = 0, ilen = changes.length; i < ilen; i++) {
        ...
        if (!allowHtml) {
          value = value.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#039;"); //escape html special chars
			...
			/*if($('#isInputShift').val() == '1') {
				var checkShiftCode = false;
				for(var i=0; i<shiftCodeDatas.length; i++) {
					if(shiftCodeDatas[i] == value.replace('+', '')) {
						checkShiftCode = true;
						break;
					}
				}
			...
			if(value.substr(0, 3) == 'ot_') {
				td.innerHTML = '<span style="color:#00BE61;">' + value.replace(/\n/g, '<br/>').replace('ot_', '') + '</span>';
			} else {
				if(col == 0) {
					var userNameSnCutStr = value.split('   ');
					var userNameStr = userNameSnCutStr[0];
					var userSnStr = userNameSnCutStr[1];//get sn of user.
					if($('#isInputName').val() == '1') {
						if(typeof(inArrayUser[userSnStr]) != 'undefined') {
							alert('已有名單');
						} else {
							inArrayUser[userSnStr] = allUserArray[userSnStr];
							td.innerHTML = userNameStr;
							$('#inputDiv').append('<input id="ScheduleUser_'+inArrayUser[userSnStr]+'" name="data[ScheduleUser]['+inArrayUser[userSnStr]+']" value="'+inArrayUser[userSnStr]+'" style="display:none;" />');
							var tmpRank = parseInt(row, 10) + 1;
							$('#inputDiv').append('<input id="ScheduleUserRank_'+inArrayUser[userSnStr]+'" name="data[ScheduleUserRank]['+inArrayUser[userSnStr]+']" value="'+tmpRank+'" style="display:none;" />');
							for(var rankNum=row; rankNum <= (parseInt(rowMax, 10) + 1); rankNum++) {
								if(typeof(datamap.data[rankNum]) != 'undefined') {
									var rankSnCutStr = datamap.data[rankNum][0].split('   ');
									if(rankSnCutStr.length > 1) {
										var rankSn = rankSnCutStr[1];
										$('#ScheduleUserRank_'+inArrayUser[rankSn]).val((parseInt(rankNum, 10) + 1));
									}
								}
							}
						}
					...
					var isTimeValue = value.split('-');
					if(isTimeValue.length > 1) {
						var timeCutStr = value.split(',');
						if(timeCutStr.length > 1) {
							//var reTime = timeStr.replace(new RegExp(' ', 'gm'), '0');
							td.innerHTML = '<div style="color:blue">' + value.replace(new RegExp(',', 'gm'), '<br/>') + '</div>';
						} else {
							td.innerHTML = '<div style="color:blue">' + value + '</div>';
						}
...
(function ($) {
  ...
  Handsontable.ColHeader.prototype.columnLabel = function (index) {
    ...
    var weekIndex = 0;
    while (dividend > 0) {
      modulo = (dividend - 1) % weekMax;
      columnLabel = weekDatas[dividend -1];
      //columnLabel = weekDatas[modulo];
      //columnLabel = String.fromCharCode(65 + modulo) + columnLabel;
      dividend = parseInt((dividend - modulo) / weekMax);
    }
    return columnLabel;
  };
