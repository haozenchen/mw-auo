...
<div id="middleLayout">
	<table class="std" cellpadding="0" width="100%">
		<tr>
			<?php if($mod_menu) : ?>
			<td valign="top">
				<div id="offsidemenu" style="display:none;"><?php echo $html->image('sidemenu_on.gif', array('title'=>'展開選單', 'border'=>'0', 'width'=>'6px','height'=>'65px', 'style'=>'cursor:pointer;', 'onclick'=>"Element.hide('offsidemenu'); Element.show('sidemenu'); Element.show('sidemenuinterval'); Element.show('onsidemenu'); Cookie.set('smenu_off', 'n')")) ?></div>
				<div id="onsidemenu" style="display:none;"><?php echo $html->image('sidemenu_off.gif', array('title'=>'收藏選單', 'border'=>'0', 'width'=>'6px', 'height'=>'65px', 'style'=>'cursor:pointer;', 'onclick'=>"Element.hide('onsidemenu'); Element.hide('sidemenu'); Element.hide('sidemenuinterval'); Element.show('offsidemenu'); Cookie.set('smenu_off', 'y');")) ?></div>
			</td>
			<td valign="top">
				<div id="sidemenu" class="sidemenu" style="display:none;">
					<table class="std" cellpadding="0" width="150px">
						<tr>
							<td>
								<?php echo $hlpHtml->sidemenuItem($mod_menu, $menuBlockKey, $menuItemKey); ?>
							</td>
						</tr>
					</table>
				</div>
			...
			<td valign="top">
				<div id="sidemenuinterval" style="width:5px;"></div>
			</td>
			<?php endif; ?>
<script>
	/**
	 * here we check menu status and use cookie to remember user setting
	 */
	var sideMenuOff = Cookie.get('smenu_off');
	if (sideMenuOff == 'y') {
		Element.hide('sidemenu');
		Element.hide('onsidemenu');
		Element.hide('sidemenuinterval');
		Element.show('offsidemenu');
	} else /*if (sideMenuOff == 'n')*/ {
		Element.hide('offsidemenu');
		Element.show('sidemenu');
		Element.show('sidemenuinterval');
		Element.show('onsidemenu');
	}
</script>
...
<script>
	$j( document ).ready(function() {				
		...
		$j('body').on('mouseover', 'span.pagecount', function(event){			
			...
			var listDiv = $j(event.target).parents('div:eq(2)').attr('id');
			if($j(event.target).parent().find('a:eq(0)').length == 0){
				return false;
			}
